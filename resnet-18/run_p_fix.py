...
for p in p_values:
    for lr in lr_values: 
        ...
        learning_rate = tf.train.exponential_decay(lr, tf.train.get_global_step() * batch_size,
                                           hp['decay_after']*train_size, 0.1, staircase=True)

        logfile = 'log_p'+str(p)+ '_lr' + str(lr)+ '_' + dataset +'.csv'


        if optimizer == 'padam':
            optim = Padam(learning_rate=learning_rate, p= p, beta1=op['b1'], beta2=op['b2'])
